...
from models.mot.fdtrack import Model
    ...
    def update(self, im_blob, img0,seq_num, save_dir):
        ...
        if len(pred) > 0:
            ...
            if len(dets) != 0:
                ...
                inds_low = dets[:, 4] < self.med_thres             #
                inds_med = dets[:, 4] > self.med_thres
                inds_high_1 = dets[:, 4] > self.opt.conf_thres+0.1
                inds_high = dets[:, 4] < self.opt.conf_thres+0.1   #

                inds_first = inds_high_1   #
                dets_first = dets[inds_first]
                inds_second = np.logical_and(inds_med, inds_high)  #
                dets_second = dets[inds_second]
                inds_third = inds_low                              #
                dets_third = dets[inds_third]

                if id_feature.shape[0] == 1:
                    id_feature_first = id_feature
                else:
                    id_feature_first = id_feature[inds_first]               
                if id_feature.shape[0] == 1:
                    id_feature_second = id_feature
                else:
                    id_feature_second = id_feature[inds_second]
                if id_feature.shape[0] == 1:
                    id_feature_third = id_feature
                else:
                    id_feature_third = id_feature[inds_third]         
                detections = []
            else:
                detections = []
                dets_first = []
        ...
        if len(dets_first) > 0:
            detections_first = [STrack(STrack.tlbr_to_tlwh(tlbrs[:4]), tlbrs[4], f, 30) for
                                 (tlbrs, f) in zip(dets_first[:, :5], id_feature_first)]
        else:
            detections_first = []
        dists = matching.iou_distance(strack_pool, detections_first)  #
        matches, u_track1, u_detection1 = matching.linear_assignment(dists, thresh=0.8)#

        for itracked, idet in matches:     #
            track = strack_pool[itracked]
            det = detections_first[idet]
            if track.state == TrackState.Tracked:   #
                track.update(detections_first[idet], self.frame_id)
        ...
        if self.opt.vis_state == 1 and self.frame_id % 20 == 0:
            if len(dets) != 0:
                ...
                track_features, det_features, cost_matrix, cost_matrix_det, cost_matrix_track = matching.vis_id_feature_A_distance(strack_pool, detections_first)
        ...
        strack_pool2 = [strack_pool[i] for i in u_track1 if strack_pool[i].state == TrackState.Tracked]
        if len(dets_second) > 0:
            detections_second = [STrack(STrack.tlbr_to_tlwh(tlbrs[:4]), tlbrs[4], f, 30) for
                                 (tlbrs, f) in zip(dets_second[:, :5], id_feature_second)]
        else:
            detections_second = []
        dists = matching.embedding_distance(strack_pool2, detections_second)#
        dists = matching.fuse_motion(self.kalman_filter, dists, strack_pool2, detections_second)  #
        matches, u_track2, u_detection2 = matching.linear_assignment(dists, thresh=0.5)#
        ...
        if self.opt.vis_state == 1 and self.frame_id % 20 == 0:
            if len(dets) != 0:
                for i in range(0, dets.shape[0]):
                    bbox = dets[i][0:4]
                    cv2.rectangle(img0, (int(bbox[0]), int(bbox[1])),(int(bbox[2]), int(bbox[3])),(0, 255, 0), 2)
                track_features, det_features, cost_matrix, cost_matrix_det, cost_matrix_track = matching.vis_id_feature_A_distance(strack_pool2, detections_second)
            vis_feature(self.frame_id,seq_num,img0,track_features,
                                  det_features, cost_matrix, cost_matrix_det, cost_matrix_track, max_num=5, out_path=save_dir)

        # association the untrack to the low score detections
        strack_pool3 = [strack_pool2[i] for i in u_track2 if strack_pool2[i].state == TrackState.Tracked]
        if len(dets_third) > 0:
            detections_third = [STrack(STrack.tlbr_to_tlwh(tlbrs[:4]), tlbrs[4], f, 30) for
                                 (tlbrs, f) in zip(dets_third[:, :5], id_feature_third)]
        else:
            detections_third = []
        dists = matching.iou_distance(strack_pool3, detections_third)
        matches, u_track3, u_detection3 = matching.linear_assignment(dists, thresh=0.4)
        for itracked, idet in matches:
            track = strack_pool3[itracked]
        ...
        for it in u_track3: #
            track = strack_pool3[it]
            if not track.state == TrackState.Lost:
                track.mark_lost()
                lost_stracks.append(track)

        '''Deal with unconfirmed tracks, usually tracks with only one beginning frame'''#
        detections = u_detection1 + u_detection2 + u_detection3
        ...
        for inew in u_detection:
            ...
            if track.score < self.opt.conf_thres:#
                continue
