...
class CCN(nn.Module):
    def __init__(self,k_size = 3,ch=()):
        ...
        #self.avg_pool_q = nn.AdaptiveAvgPool2d((w, h))
        
        self.senetd1 = nn.Sequential(nn.Linear(ch, ch // 16, bias=False),
                                   nn.ReLU(),
                                   nn.Linear(ch // 16, ch, bias=False),
                                   nn.Sigmoid()
        )
        self.senetd2 = nn.Sequential(nn.Linear(ch, ch // 16, bias=False),
                                   nn.ReLU(),
                                   nn.Linear(ch // 16, ch, bias=False),
                                   nn.Sigmoid()
        )
        self.senetr1 = nn.Sequential(nn.Linear(ch, ch // 16, bias=False),
                                   nn.ReLU(),
                                   nn.Linear(ch // 16, ch, bias=False),
                                   nn.Sigmoid()
        )
        self.senetr2 = nn.Sequential(nn.Linear(ch, ch // 16, bias=False),
                                   nn.ReLU(),
                                   nn.Linear(ch // 16, ch, bias=False),
                                   nn.Sigmoid()
        )

        self.q_attention1 = nn.Sequential(nn.Conv2d(ch, ch, kernel_size=3, stride=1, padding=1, bias=True),
                                          nn.InstanceNorm2d(num_features=ch),
                                          nn.LeakyReLU(0.3, inplace=True))
        self.q_attention2 = nn.Sequential(nn.Conv2d(ch, ch, kernel_size=3, stride=1, padding=1, bias=True),
                                          nn.InstanceNorm2d(num_features=ch),
                                          nn.LeakyReLU(0.3, inplace=True))
    ...
    def forward(self, x):
        ...
        #senet
        avg1 = self.avg_pool1(x).view(b, c)
        max1 = self.max_pool1(x).view(b, c)
        se1=self.senetd1(avg1).view(b,c,1,1)
        se2=self.senetd2(max1).view(b,c,1,1)
        se1=se1*se2
        se3=self.senetr1(avg1).view(b,c,1,1)
        se4=self.senetr2(max1).view(b,c,1,1)
        se2=se3*se4
        se1=x * se1.expand_as(x)
        se2=x * se2.expand_as(x)
        
        #
        q = self.avg_pool(x)
        q1=self.q_attention1(q)
        q2=self.q_attention2(q)
        bs, c, h, w = q1.shape
        q1 = q1.view(bs, c, h * w)#reshape
        q2 = q2.view(bs, c, h * w)

        q1_T = q1.permute(0, 2, 1)#transpose
        q2_T = q2.permute(0, 2, 1)
        q_s1 = torch.matmul(q1, q2_T)
        q_s2 = torch.matmul(q2, q1_T)
        q_s1 = F.softmax(q_s1, dim=-1)#w
        q_s2 = F.softmax(q_s2, dim=-1)

        x_q1 = se1
        x_q2 = se2
        bs, c, h, w = x_q1.shape
        x_q1 = x_q1.contiguous().view(bs, c, h * w)#
        x_q2 = x_q2.contiguous().view(bs, c, h * w)#reshape
        x_q1 = torch.matmul( q_s1, x_q1).contiguous().view(bs, c, h, w)
        x_q2 = torch.matmul( q_s2, x_q2).contiguous().view(bs, c, h, w)
        
        x_q1=x_q1+x
        x_q2=x_q2+x
        
        # scm
        y1 = self.avg_pool(x_q1)
        y2 = self.avg_pool(x_q2)

        y_t1 = self.c_attention1(y1)
        y_t2 = self.c_attention2(y2)
        bs,c,h,w = y_t1.shape
        y_t1 =y_t1.view(bs, c, h*w)
        y_t2 =y_t2.view(bs, c, h*w)

        y_t1_T = y_t1.permute(0, 2, 1)#transpose
        ...
        x_t1 = torch.matmul(M_t1 , x_t1).contiguous().view(bs, c, h, w)
        x_t2 = torch.matmul(M_t2 , x_t2).contiguous().view(bs, c, h, w)
        #print("M_t1",torch.sort(M_t1[0][0]))
        #print("y_t1",torch.max(y_t1),torch.min(y_t1))
        #print("y_t2", torch.max(y_t2), torch.min(y_t2))
        return [x_t1+x_q1,x_t2+x_q2]
