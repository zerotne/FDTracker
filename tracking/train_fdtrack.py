...
from core.mot import eval  # import test.py to get mAP after each epoch
from models.mot.fdtrack import Model
from dataset.fdtrack import create_dataloader
from core.mot.general import (
    torch_distributed_zero_first, labels_to_class_weights, plot_labels, check_anchors, labels_to_image_weights,
    plot_images, fitness, strip_optimizer, plot_results, get_latest_run, check_dataset, check_file,
    check_git_status, check_img_size, increment_dir, print_mutation, plot_evolution, set_logging)
from core.mot.google_utils import attempt_download
from core.mot.base_trainer import compute_loss,MotLoss
from core.mot.torch_utils import init_seeds, ModelEMA, select_device, intersect_dicts
from core.mot.optimizer import Lookahead
...
#
    ...
    dataloader, dataset = create_dataloader(dataset_root,trainset_paths, imgsz, batch_size, gs, opt, hyp=hyp, augment=True,
                                            cache=opt.cache_images, rect=False, rank=rank,
                                            world_size=opt.world_size, workers=opt.workers,state="train")
    ...
    if rank in [-1, 0]:#
        ...
        testloader = create_dataloader(dataset_root,test_paths, imgsz, total_batch_size, gs, opt, hyp=hyp, augment=False,
                                       cache=opt.cache_images, rect=True, rank=-1, world_size=opt.world_size,
                                       workers=opt.workers,state="test")[0]
    ...
    accumulate = max(round(nbs / total_batch_size), 1)  # accumulate loss before optimizin
    ...
    for epoch in range(start_epoch, epochs):  #  epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if opt.multi_scale and ni > nw and epoch%2 == 0:
                ...
                sz = random.randrange(imgsz[0] * 0.5, imgsz[0] * 1.0) // gs * gs  # size
...
if __name__ == '__main__':
    ...
    #parser.add_argument('--weights', type=str, default='../weights/best_mot_test.pt', help='initial weights path')
    parser.add_argument('--weights', type=str, default='', help='initial weights path')
    parser.add_argument('--cfg', type=str, default='../experiments/model_set/FDTrack.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default='../lib/dataset/mot/cfg/data_my.json', help='data.json path')

    parser.add_argument('--hyp', type=str, default='../experiments/train/FDTrack.yaml', help='hyperparameters path, i.e. data/hyp.scratch.yaml')#超参数设定
    parser.add_argument('--epochs', type=int, default=100)
    parser.add_argument('--start_epoch', type=int, default=0)
    parser.add_argument('--batch_size', type=int, default=16
                        , help='total batch size for all GPUs')
    parser.add_argument('--img-size', nargs='+', type=int, default=[1088, 608], help='train,test sizes')
    ...
    parser.add_argument('--name', default='mot_test', help='renames results.txt to results_name.txt if supplied')#
    ...
    parser.add_argument('--fineturn', action='store_true', help='fineturn on MOT')#
    ...
    parser.add_argument('--emb_dim', type=int, default=512, help='Eigenvector length of id feature')#
    ...
    if not opt.evolve:#
        tb_writer = None
        if opt.global_rank in [-1, 0]:
            logger.info('Start Tensorboard with "tensorboard --logdir %s", view at http://localhost:1234/' % opt.logdir)
